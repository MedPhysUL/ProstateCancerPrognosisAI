...
class FeatureSelector:
    ...
    def __init__(
        ...
        seed : Optional[int]
            Number used as a random state for the random forest doing feature selection.
        """
        for t in threshold:
            if not 0 < t <= 1:
    ...
    def __call__(
        ...
        Returns : Union[
            ...
            Tuple of the list of cont_cols preserved, the list of cat_cols preserved and, sometimes the dict of features
            importance.
        """
    ...
    def get_features_importance(
        ...
        for i, group in enumerate(dataset.feature_selection_idx_groups.values()):
            ...
            fi_table = pd.DataFrame({'features': features,
        ...
        fi_table = pd.concat(fi_tables, axis=0, ignore_index=True)
