...
from monai.transforms import Compose, ToTensord, EnsureChannelFirstd, CenterSpatialCropd
...
if __name__ == "__main__":
    ...
    ax["A"].imshow(layers.img[0][0][int(size / 2), :, :], cmap="Greys_r")
    ax["A"].set_title(f"img {np.shape(layers.img)}")

    ax["B"].imshow(layers.enc1[0][0][int(size / 4), :, :], cmap="Greys_r")
    ax["B"].set_title(f"enc1 {np.shape(layers.enc1)}")

    ax["C"].imshow(layers.enc2[0][0][int(size / 8), :, :], cmap="Greys_r")
    ax["C"].set_title(f"enc2 {np.shape(layers.enc2)}")

    ax["D"].imshow(layers.enc3[0][0][int(size / 16), :, :], cmap="Greys_r")
    ax["D"].set_title(f"enc3 {np.shape(layers.enc3)}")

    ax["E"].imshow(layers.enc4[0][0][int(size / 32), :, :], cmap="Greys_r")
    ax["E"].set_title(f"enc4 {np.shape(layers.enc4)}")

    ax["F"].imshow(layers.bottom[0][0][int(size / 32), :, :], cmap="Greys_r")
    ax["F"].set_title(f"bottom {np.shape(layers.bottom)}")

    ax["G"].imshow(layers.dec4[0][0][int(size / 16), :, :], cmap="Greys_r")
    ax["G"].set_title(f"dec4 {np.shape(layers.dec4)}")

    ax["H"].imshow(layers.dec3[0][0][int(size / 8), :, :], cmap="Greys_r")
    ax["H"].set_title(f"dec3 {np.shape(layers.dec3)}")

    ax["I"].imshow(layers.dec2[0][0][int(size / 4), :, :], cmap="Greys_r")
    ax["I"].set_title(f"dec2 {np.shape(layers.dec2)}")

    ax["J"].imshow(layers.dec1[0][0][int(size / 2), :, :], cmap="Greys_r")
    ax["J"].set_title(f"dec1 {np.shape(layers.dec1)}")

    ax["K"].imshow(out[0][0][int(size / 2), :, :], cmap="Greys_r")
