...
if __name__ == '__main__':
    split = 2
    df = pd.read_csv(
        fr"C:\Users\Labo\Documents\GitHub\ProstateCancerPrognosisAI\applications\local_data\radiomics\multitask\outer_split_{split}\outer_split.csv"
    )
    ...
    model = SequentialNet(
        ...
        n_layers={PN_TASK.name: 3, BCR_TASK.name: 3, METASTASIS_TASK.name: 3, HTX_TASK.name: 2, CRPC_TASK.name: 2, DEATH_TASK.name: 3},
        n_neurons={PN_TASK.name: 20, BCR_TASK.name: 20, METASTASIS_TASK.name: 20, HTX_TASK.name: 20, CRPC_TASK.name: 15, DEATH_TASK.name: 15},
        features_columns={
    ...
    state = torch.load(
        rf"C:\Users\Labo\Documents\GitHub\ProstateCancerPrognosisAI\applications\local_data\records\experiments\DEATH(SeqNet - Clinical data only)\outer_splits\split_{split}\best_models\outer_split\best_model.pt"
    )
    ...
    import json
    path = r'C:\Users\Labo\Desktop\temporary_save_folder\preds\split_2\SeqNet.json'
