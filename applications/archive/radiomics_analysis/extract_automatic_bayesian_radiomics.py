...
from constants import (
    ...
    AUTOMATIC_BAYESIAN_EXTRACTED_RADIOMICS_PATH,
...
if __name__ == '__main__':
    ...
    for k in range(5):
        model_state = torch.load(
            os.path.join(AUTOMATIC_BAYESIAN_RADIOMICS_MODELS_PATH, f"outer_split_{k}", "model.pth")
        )["model_state"]
        ...
        radiomics_dataset = RadiomicsDataset(
            path_to_dataset=os.path.join(AUTOMATIC_BAYESIAN_EXTRACTED_RADIOMICS_PATH, f"ct_outer_split_{k}.csv")
        )
        save_radiomics(dataset, radiomics_dataset, "CT")

        radiomics_dataset = RadiomicsDataset(
            path_to_dataset=os.path.join(AUTOMATIC_BAYESIAN_EXTRACTED_RADIOMICS_PATH, f"pt_outer_split_{k}.csv")
        )
