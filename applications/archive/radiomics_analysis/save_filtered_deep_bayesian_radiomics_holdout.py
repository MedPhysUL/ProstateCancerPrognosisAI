...
from constants import (
    ...
    DEEP_BAYESIAN_RADIOMICS_HOLDOUT_PATH,
    ID,
    HOLDOUT_MASKS_PATH,
    LEARNING_TABLE_PATH,
    LEARNING_SET_PATH,
...
if __name__ == "__main__":
    ...
    os.makedirs(DEEP_BAYESIAN_RADIOMICS_HOLDOUT_PATH, exist_ok=True)
    ...
    model = UNEXtractor(
        ...
        bayesian=True,
        temperature={BCR_TASK.name: 0.01, PROSTATE_SEGMENTATION_TASK.name: 1e-4},
        seed=SEED
    ).build(dataset)

    loaded_state = torch.load(
        r"local_data\records\experiments\HOLDOUT\BCR(BayesUNEXtractor - Deep radiomics)\best_model_checkpoint.pt"
    )["model_state"]
    ...
    for features, _ in data_loader:
        rad = []
        for i in range(100):
            rad.append(model.extract_radiomics(features).deep_features.cpu().detach().numpy())
        rads.append(np.mean(rad, axis=0))
    ...
    dataframe.to_csv(os.path.join(DEEP_BAYESIAN_RADIOMICS_HOLDOUT_PATH, f"learning.csv"), index=False)
