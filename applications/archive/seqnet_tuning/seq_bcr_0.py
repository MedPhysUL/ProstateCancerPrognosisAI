...
if __name__ == '__main__':
    ...
    config_dict = get_model_configs_dictionary(
        path_to_outer_splits_folder=os.path.join(
            EXPERIMENTS_PATH,
            r"PN(MLP - Clinical data and automatic radiomics)\outer_splits"
        ),
        n_outer_loops=5,
        n_inner_loops=5
    )
    ...
    model_hyperparameter = TorchModelHyperparameter(
        ...
        parameters={
            ...
            "dropout": HyperparameterDict(
                {
                    PN_TASK.name: 0.05,
                    BCR_TASK.name: FloatHyperparameter(name="dropout", low=0.05, high=0.25)
                }
            )
        }
    )
    ...
    learning_algorithm_hyperparameter = LearningAlgorithmHyperparameter(
        ...
        clip_grad_max_norm=PREDICTOR_CLIP_GRAD_MAX_NORM_DICT[BCR_TASK.target_column],
