...
def get_model_configs_dictionary(
        path_to_outer_splits_folder: str,
        n_outer_loops: int,
        n_inner_loops: int
    ...
    configs = {}
    for k in range(n_outer_loops):
        path_to_best_models = os.path.join(path_to_outer_splits_folder, f"split_{k}", "best_models")
...
if __name__ == '__main__':
    ...
    config_dict = get_model_configs_dictionary(
        path_to_outer_splits_folder=r"C:\Users\maxen\Documents\GitHub\ProstateCancerPrognosisAI\applications\local_data\records\experiments\PN(MLP - Clinical data and automatic radiomics)\outer_splits",
        n_outer_loops=5,
        n_inner_loops=5
    )
    ...
    table_dataset = TableDataset(
        ...
        ids_column=ID,
        tasks=[PN_TASK, METASTASIS_TASK],
        continuous_features=CLINICAL_CONTINUOUS_FEATURES + PN_RADIOMICS,
    ...
    model_hyperparameter = TorchModelHyperparameter(
        ...
        parameters={
            ...
            "features_columns": {
                ...
                METASTASIS_TASK.name: [c.column for c in CLINICAL_CONTINUOUS_FEATURES] + [
                    c.column for c in CLINICAL_CATEGORICAL_FEATURES]
            },
            "dropout": HyperparameterDict(
                {
                    PN_TASK.name: 0.05,
                    METASTASIS_TASK.name: FloatHyperparameter(name="dropout", low=0.05, high=0.25)
                }
            )
        }
    )
    ...
    learning_algorithm_hyperparameter = LearningAlgorithmHyperparameter(
        criterion=CriterionHyperparameter(
            ...
            parameters={"tasks": METASTASIS_TASK}
        ),
        ...
        clip_grad_max_norm=PREDICTOR_CLIP_GRAD_MAX_NORM_DICT[METASTASIS_TASK.target_column],
