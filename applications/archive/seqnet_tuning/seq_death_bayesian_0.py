...
from constants import (
    ...
    MULTITASK_BAYESIAN_RADIOMICS_PATH
)
...
if __name__ == '__main__':
    ...
    df_dict = get_dataframes_dictionary(
        path_to_dataframes_folder=MULTITASK_BAYESIAN_RADIOMICS_PATH,
        n_outer_loops=5,
        n_inner_loops=5
    )
    ...
    config_dict = get_model_configs_dictionary(
        path_to_outer_splits_folder={
            PN_TASK: os.path.join(EXPERIMENTS_PATH, "PN_BayesMLP_Clinical_data_and_automatic_bayesian_radiomics", "outer_splits"),
            BCR_TASK: os.path.join(EXPERIMENTS_PATH, "BCR_BayesSeqNet_Clinical_data_and_deep_radiomics", "outer_splits"),
            METASTASIS_TASK: os.path.join(EXPERIMENTS_PATH, "METASTASIS_BayesSeqNet_Clinical_data_only", "outer_splits"),
            HTX_TASK: os.path.join(EXPERIMENTS_PATH, "HTX_BayesSeqNet_Clinical_data_only", "outer_splits"),
            CRPC_TASK: os.path.join(EXPERIMENTS_PATH, "CRPC_BayesSeqNet_Clinical_data_only", "outer_splits"),
        },
        n_outer_loops=5,
        n_inner_loops=5
    )
    ...
    table_dataset = TableDataset(
        ...
        ids_column=ID,
        tasks=[PN_TASK, BCR_TASK, METASTASIS_TASK, HTX_TASK, CRPC_TASK, task],
    ...
    path_to_record_folder = os.path.join(
        ...
        f"{task.target_column}(SequentialNET - FirstSplit)"
    )
    ...
    model_hyperparameter = TorchModelHyperparameter(
        ...
        parameters={
            "sequence": [
                PN_TASK.name, BCR_TASK.name, METASTASIS_TASK.name, HTX_TASK.name, CRPC_TASK.name, task.name
            ],
            ...
            "n_layers": HyperparameterDict(
                {
                    ...
                    HTX_TASK.name: 2,
                    CRPC_TASK.name: 2,
                    task.name: IntegerHyperparameter(
                        name="n_layers",
                        low=1,
                        high=3
                    )
            ...
            "features_columns": {
                ...
                task.name: [c.column for c in CLINICAL_CONTINUOUS_FEATURES] + [c.column for c in CLINICAL_CATEGORICAL_FEATURES]
            },
            "dropout": HyperparameterDict(
                {
                    PN_TASK.name: 0.19835961630626225,
                    BCR_TASK.name: 0.22303674118039118,
                    METASTASIS_TASK.name: 0.24999999999999997,
                    HTX_TASK.name: 0.12256963151781752,
                    CRPC_TASK.name: 0.0582654598850484,
                    task.name: FloatHyperparameter(name="dropout", low=0.05, high=0.25)
                }
            ),
            "bayesian": True,
            "temperature": HyperparameterDict({
                PN_TASK.name: 1e-4,
                BCR_TASK.name: 0.00011853670530788392,
                METASTASIS_TASK.name: 0.00012131944187604591,
                HTX_TASK.name: 0.0005652823161291345,
                CRPC_TASK.name: 0.004581680852001114,
