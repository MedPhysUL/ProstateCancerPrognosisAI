...
if __name__ == '__main__':
    ...
    table_dataset = TableDataset(
        ...
        ids_column=ID,
        tasks=[PN_TASK, BCR_TASK, METASTASIS_TASK, HTX_TASK],
    ...
    model_hyperparameter = TorchModelHyperparameter(
        ...
        parameters={
            "sequence": [
                PN_TASK.name, BCR_TASK.name, METASTASIS_TASK.name, HTX_TASK.name
            ],
            ...
            "n_layers": HyperparameterDict(
                {
                    ...
                    HTX_TASK.name: IntegerHyperparameter(
                        name="n_layers",
                        low=1,
                        high=3
                    )
            ...
            "dropout": HyperparameterDict(
                {
                    ...
                    HTX_TASK.name: FloatHyperparameter(name="dropout", low=0.05, high=0.25)
                }
            )
        }
    )
    ...
    learning_algorithm_hyperparameter = LearningAlgorithmHyperparameter(
        criterion=CriterionHyperparameter(
            ...
            parameters={"tasks": HTX_TASK}
        ),
        optimizer=OptimizerHyperparameter(
            constructor=Adam,
            model_params_getter=lambda model: model.predictor[HTX_TASK.name].parameters(),
            parameters={
        ...
        clip_grad_max_norm=PREDICTOR_CLIP_GRAD_MAX_NORM_DICT[HTX_TASK.target_column],
        ...
        regularizer=RegularizerHyperparameter(
            ...
            model_params_getter=lambda model: model.predictor[HTX_TASK.name].named_parameters(),
