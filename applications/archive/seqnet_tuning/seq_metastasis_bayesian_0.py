...
if __name__ == '__main__':
    ...
    outer_splits_folder = os.path.join(
        EXPERIMENTS_PATH,
        "PN_BayesMLP_Clinical_data_and_automatic_bayesian_radiomics",
        "outer_splits"
    )

    config_dict = get_model_configs_dictionary(
        path_to_outer_splits_folder=outer_splits_folder,
        n_outer_loops=5,
        n_inner_loops=5
    )
    ...
    model_hyperparameter = TorchModelHyperparameter(
        ...
        parameters={
            ...
            "features_columns": {
                ...
                task.name: [c.column for c in CLINICAL_CONTINUOUS_FEATURES] + [
                    c.column for c in CLINICAL_CATEGORICAL_FEATURES]
            },
            ...
            "temperature": HyperparameterDict({
                ...
                PN_TASK.name: FixedHyperparameter(name="tempPN", value=1e-4)
            }),
        }
    )
