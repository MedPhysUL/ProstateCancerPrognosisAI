...
if __name__ == '__main__':
    ...
    config_dict = get_model_configs_dictionary(
        path_to_outer_splits_folder={
            PN_TASK: os.path.join(EXPERIMENTS_PATH, "PN(MLP - Clinical data and automatic radiomics)", "outer_splits"),
            BCR_TASK: os.path.join(EXPERIMENTS_PATH, "BCR(SeqNet - Clinical data and deep radiomics)", "outer_splits"),
            METASTASIS_TASK: os.path.join(EXPERIMENTS_PATH, "METASTASIS(SeqNet - Clinical data only)", "outer_splits"),
            HTX_TASK: os.path.join(EXPERIMENTS_PATH, "HTX(SeqNet - Clinical data only)", "outer_splits"),
            CRPC_TASK: os.path.join(EXPERIMENTS_PATH, "CRPC(SeqNet - Clinical data only)", "outer_splits")
        },
        n_outer_loops=5,
        n_inner_loops=5
    )
    ...
    path_to_record_folder = os.path.join(
        EXPERIMENTS_PATH,
        f"DEATH(SeqNet - Clinical data only - Split0)"
    )
    ...
    model_hyperparameter = TorchModelHyperparameter(
        ...
        parameters={
            "sequence": [
                PN_TASK.name, BCR_TASK.name, METASTASIS_TASK.name, HTX_TASK.name, CRPC_TASK.name, DEATH_TASK.name
            ],
            ...
            "dropout": HyperparameterDict(
                {
                    ...
                    CRPC_TASK.name: 0.176565372697351,
                    DEATH_TASK.name: FloatHyperparameter(name="dropout", low=0.05, high=0.25)
                }
            )
        }
    )
    ...
    learning_algorithm_hyperparameter = LearningAlgorithmHyperparameter(
        ...
        clip_grad_max_norm=PREDICTOR_CLIP_GRAD_MAX_NORM_DICT[DEATH_TASK.target_column],
